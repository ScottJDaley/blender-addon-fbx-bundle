...
class FBXBundleSettings(bpy.types.PropertyGroup):
    ...
    collapseBundles: bpy.props.BoolProperty(
        name="Collapse", default=True, description="Compact list view"
    )
    ...
    recent: bpy.props.StringProperty(name="Recent export", default="")
    ...
    mode_pivot: bpy.props.EnumProperty(
        items=[
            ...
            (
                "BOUNDS_BOTTOM",
                "Bottom Center",
                "Pivot at the bottom center of the bounds of the bundle",
            ),
            ...
            (
                "EMPTY",
                "Empty Gizmo",
                "Empty gizmo object of: Arrow, Plain Axes, Single Arrow",
            ),
        ],
        name="Pivot From",
        default="PARENT",
    )
    target_platform: bpy.props.EnumProperty(
        items=[
...
class Panel_Core(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row.prop(
            bpy.context.scene.FBXBundleSettings,
            "target_platform",
            text="",
            icon_value=icon,
        )
        ...
        ).url = "http://renderhjs.net/fbxbundle/#settings_platform"
        ...
        row = col.row(align=True)
        row.prop(
            context.scene.FBXBundleSettings,
            "mode_bundle",
            text="Bundle by",
            icon="GROUP",
        )
        row.operator(
            "wm.url_open", text="", icon="QUESTION"
        ).url = "http://renderhjs.net/fbxbundle/#settings_bundle"
        ...
        row.operator(
            "wm.url_open", text="", icon="QUESTION"
        ).url = "http://renderhjs.net/fbxbundle/#settings_pivot"

        col = box.column(align=True)
        row = col.row(align=True)
        row.prop(
            context.scene.FBXBundleSettings, "padding", text="Padding", expand=True
        )
        row.prop(
            context.scene.FBXBundleSettings,
            "include_children",
        ...
        if context.scene.FBXBundleSettings.path == "":
            ...
            context.scene.FBXBundleSettings.mode_bundle == "GROUP"
            and len(bpy.data.groups) == 0
        ):
            box = col.box()
...
class Panel_Tools(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        col.operator(
            op_pivot_ground.op.bl_idname,
            text="Pivot at Ground",
            icon="OUTLINER_DATA_EMPTY",
        )
        col.operator(
            op_tool_geometry_fix.op.bl_idname,
            text="Fix imp. Geometry",
            icon="MESH_ICOSPHERE",
        )

        if bpy.app.debug_value != 0:
            col.operator(
                op_tool_pack_bundles.op.bl_idname,
                text="Pack & Arrange",
                icon="UGLYPACKAGE",
            )
            ...
            row.operator(
                op_fence_clear.op.bl_idname, text="Align Z", icon="TRIA_DOWN_BAR"
            )
            layout.separator()
...
class Panel_Files(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        c.scale_y = 1.85
        c.operator(
            op_file_export.op.bl_idname,
            text="Export {}x".format(len(bundles)),
            icon_value=icon,
        )
        ...
        if len(bpy.context.scene.FBXBundleSettings.recent) > 0:
            if len(objects_organise.recent_load_objects()) > 0:
                ...
                r.operator(
                    op_file_export_recent.op.bl_idname,
                    text=objects_organise.recent_get_label(),
                    icon="RECOVER_LAST",
                )
        ...
        if len(bundles) > 0:
            ...
            folder = os.path.dirname(
                bpy.path.abspath(bpy.context.scene.FBXBundleSettings.path)
            )
            ...
            for fileName, objects in bundles.items():
                ...
                row.operator(
                    op_select.bl_idname, icon_value=icon, emboss=False, text=label
                ).key = fileName
                r = row.row(align=True)
                r.alert = True
...
def register():
    ...
    bpy.types.Scene.FBXBundleSettings = bpy.props.PointerProperty(
        type=FBXBundleSettings
    )
    ...
    kmi = km.keymap_items.new(
        op_file_export.op.bl_idname, "E", "PRESS", ctrl=True, shift=False
    )
    kmi = km.keymap_items.new(
        op_file_export_recent.op.bl_idname, "E", "PRESS", ctrl=True, shift=True
    )
